function [path] = RRT_start(start_pt,end_pt,radius,obs_pos_x,obs_pos_y,stepsize)
...
for sits = 1:size(stepsize,2)
    segmentLength = stepsize(sits); 
        ...
        avg_path = 0;
        for i = 1:num_of_runs
            [n_its, path_n,run_time,path] = RRTstar_alg(dim,segmentLength,radius,...
                random_world,start_pt,end_pt,show_output,samples,obs_pos_x,obs_pos_y);
            time = time + run_time;
