function fixed_and_dynamic_obs()
...
clns = repmat({':'}, 1, gDim);
obs_start_lower = [1,-5];
obs_start_upper = -obs_start_lower;
obs_end_lower = [4,4];
obs_end_upper = -obs_end_lower;


obs_lower_1 = linspace(obs_start_lower(1), obs_end_lower(1),length(tau))';
obs_lower_2 = (sin(obs_lower_1));
obs_lower = [obs_lower_1 obs_lower_2+3];

obs_upper_1 = linspace(obs_end_upper(1), obs_start_upper(1),length(tau))';
obs_upper_2 = cos(obs_upper_1);
obs_upper = [flip(obs_upper_1) obs_upper_2];
R = 1;
for ii = 1:length(tau)
    %HJIextraArgs.obstacleFunction
    obstacles_d1(clns{:}, ii) = shapeCylinder(g,3, obs_upper(ii,:), R);
    obstacles_d2(clns{:}, ii) = shapeCylinder(g,3, obs_lower(ii,:), R);
    obstacles_f1(clns{:}, ii) = obstacles_1;
    obstacles_f2(clns{:}, ii) = obstacles_2;
end

obstacles_f = shapeUnion(obstacles_f1,obstacles_f2);
obstacles_d = shapeUnion(obstacles_d1,obstacles_d2);
obstacles = shapeUnion(obstacles_f,obstacles_d);
...
if compTraj
  ...
  if value <= 0 %if initial state is in BRS/BRT
    ...
    hold on
    [g2D_of,data2D_of] = proj(g,obstacles_f,[0 0 1]);
