function [traj,dt] = HJ_avoid_obs_test(detect,Init_pos,target_pos,radius_tar,t_gap,v,radius_obs,F_obs,Num_F,D_obs,Num_D,U_obs,Num_U)
...
for n = 1:length(detect)
    if flag == 0
        if detect(n) <= Num_F % Fix
            ...
            Tmp = shapeCylinder(g, 3, [U_obs(1,detect(n)); U_obs(2,detect(n)); 0], radius_obs);
        end
        ...
        elseif detect(n) <= (Num_F + Num_D) && detect(n) > Num_F % Dyn
            for ii = 1: length(tau)
                eval(['Tmp(clns{:}, ii) = shapeCylinder(g, 3, [D_obs(1,n) + dt * (ii-1); g_func',...
                    num2str(detect(n) - Num_F),'(D_obs(1,n) + dt * (ii-1)); 0], radius_obs);'])
            end
        else % Unk
            for ii = 1:length(tau)
                Tmp(clns{:}, ii) = shapeCylinder(g, 3, [U_obs(1,detect(n)); U_obs(2,detect(n)); 0], radius_obs);
            end
        end
    end
...
if compTraj
  ...
  xinit = [Init_pos(1),Init_pos(2), pi/3];
